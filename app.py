...
from models import db, students, Mark
from urllib.parse import quote_plus
from flask_migrate import Migrate

app = Flask(__name__)

app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:%s@localhost/stud'% quote_plus('Ranjith@45')
...
def add_student():
    ...
    student = students(roll_no=roll_number, name=name, dob=dob)
...
def add_marks(pk):
    ...
    stud = Mark(marks= marks, student_id=pk)
    db.session.add(stud)
    db.session.commit()
...
def get_results():
    ...
    pass_count = 0
    total_students = len(stud)
    for student in stud:
        marks = Mark.query.filter_by(student_id=student.id).first()
        if marks:
